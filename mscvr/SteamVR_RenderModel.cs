...
public class SteamVR_RenderModel : MonoBehaviour
{
		...
		public CVRRenderModels instance
		{
			get
			{
				if (_instance == null && !failedLoadInterface)
				{
					...
					if (_instance == null)
					{
                        MSCLoader.ModConsole.Print("Failed to load IVRRenderModels interface version " + OpenVR.IVRRenderModels_Version);
	...
	public void UpdateModel()
	{
		...
		if (capacity <= 1)
		{
            MSCLoader.ModConsole.Print("Failed to get render model name for tracked object " + index);
			return;
		}
	...
	private bool SetModel(string renderModelName)
	{
		...
		using (var holder = new RenderModelInterfaceHolder())
		{
			if (createComponents)
			{
                ...
                MSCLoader.ModConsole.Print("[" + gameObject.name + "] Render model does not support components, falling back to single mesh.");
			}
			...
			if (!string.IsNullOrEmpty(renderModelName))
			{
				...
				if (model == null || model.mesh == null)
				{
					...
					if (verbose)
                        MSCLoader.ModConsole.Print("Loading render model " + renderModelName);
	...
	RenderModel LoadRenderModel(CVRRenderModels renderModels, string renderModelName, string baseName)
	{
		...
		{
            MSCLoader.ModConsole.Print(string.Format("Failed to load render model {0} - {1}", renderModelName, error.ToString()));
			return null;
		}
		...
		if (material == null || material.mainTexture == null)
		{
			...
			{
                MSCLoader.ModConsole.Print("Failed to load render model texture for render model " + renderModelName);
			}
		}
	...
	private bool LoadComponents(RenderModelInterfaceHolder holder, string renderModelName)
	{
		...
		for (int i = 0; i < count; i++)
		{
			...
			{
				if (verbose)
                    MSCLoader.ModConsole.Print("Loading render model " + componentRenderModelName);
	...
	void OnEnable()
	{
		...
		{
            MSCLoader.ModConsole.Print(modelOverrideWarning);
			enabled = false;
			return;
		}
