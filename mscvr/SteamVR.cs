...
public class SteamVR : System.IDisposable
{
	...
	static SteamVR CreateInstance()
	{
		try
		{
			...
			{
				MSCLoader.ModConsole.Print("OpenVR initialization failed.  Ensure 'Virtual Reality Supported' is checked in Player Settings, and OpenVR is added to the list of Virtual Reality SDKs.");
				return null;
			}
	...
	static void ReportError(EVRInitError error)
	{
		switch (error)
		{
			...
			case EVRInitError.VendorSpecific_UnableToConnectToOculusRuntime:
                MSCLoader.ModConsole.Print("SteamVR Initialization Failed!  Make sure device is on, Oculus runtime is installed, and OVRService_*.exe is running.");
				break;
			case EVRInitError.Init_VRClientDLLNotFound:
                MSCLoader.ModConsole.Print("SteamVR drivers not found!  They can be installed via Steam under Library > Tools.  Visit http://steampowered.com to install Steam.");
				break;
			case EVRInitError.Driver_RuntimeOutOfDate:
                MSCLoader.ModConsole.Print("SteamVR Initialization Failed!  Make sure device's runtime is up to date.");
				break;
			default:
                MSCLoader.ModConsole.Print(OpenVR.GetStringForHmdError(error));
				break;
		}
	}
